// =============================================================================
// NKR Pull — Uses Docker (extractor) to build ext4 disks from OCI images
// =============================================================================
...
pub fn pull_image(image: &str, dest: &str, size_mb: u32) -> Result<(), Box<dyn Error>> {
    ...
    // 2. Export the container to a local tar
    ...
    // 3. Create empty disk file (+C on btrfs before allocate)
    ...
    // 5. Extract the tar into the mounted disk
...
/// Extracts ENTRYPOINT and CMD from a Docker image, returns the combined list
...
/// Parses a simple JSON array like ["a","b"]
...
/// Drops the disk in /mnt/nkr/images/ and auto-generates the initramfs.
...
pub fn pull_and_generate(
    ...
    // Derive disk name from the image: "edoburu/pgbouncer:latest" → "pgbouncer"
    let raw_name = image
        .rsplit('/')
        .next()
        .unwrap_or(image)
    ...
    // 2. Extract full Docker CMD (ENTRYPOINT + CMD)
